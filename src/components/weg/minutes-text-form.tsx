...
export function MinutesTextForm({ hoaId, meetingId, minutesText }: { hoaId: string; meetingId: string; minutesText: string | null }) {
	...
	return (
		<form action={formAction} className="space-y-3">
			...
			<Textarea name="minutesText" className="min-h-48" placeholder="Verlauf der Versammlung, Anwesenheit, Diskussionspunkte..." defaultValue={minutesText ?? ""} />
			{state.error ? <p className="text-sm text-destructive">{state.error}</p> : null}
			<div className="flex justify-end">
				<Button type="submit" size="sm" disabled={isPending}>
					{isPending ? <Loader2 className="animate-spin" /> : <Save />}
					Protokolltext speichern
				</Button>
			</div>
		</form>
	);
