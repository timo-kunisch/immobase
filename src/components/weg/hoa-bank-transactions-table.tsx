...
import { BankTransactionAllocateDialog, type OpenHousingChargeOption } from "@/components/buchhaltung/bank-transaction-allocate-dialog";
...
/**
 * Banktransaktions-Tabelle der WEG-Buchhaltung (/weg/buchhaltung, Tab
 * "Banktransaktionen"): clientseitige Sortierung/Filterung inkl.
 * Status-Select-Filter und Client-Pagination (50/Seite). Konten und offene
 * Hausgeld-Sollstellungen kommen als Props aus der Server-Seite (für den
 * Zuordnen-Dialog, Buchungskreis WEG - offene Miet-Sollstellungen bleiben
 * der Mietverwaltung vorbehalten).
 */
export function HoaBankTransactionsTable({
	rows,
	accounts,
	openHousingCharges,
}: {
	...
	openHousingCharges: OpenHousingChargeOption[];
}) {
	const { t } = useI18n();
			...
			cell: (row) => (
				<div className="flex items-center justify-end gap-1">
					<BankTransactionAllocateDialog
						...
						accounts={accounts}
						openTransactions={[]}
						housingCharges={openHousingCharges}
					/>
