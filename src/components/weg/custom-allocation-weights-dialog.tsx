	...
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogTrigger asChild>
				<Button variant="ghost" size="icon-sm" aria-label={t("hoa.allocationKeys.weights.open")} title={t("hoa.allocationKeys.weights.open")}>
			...
			<DialogContent className="sm:max-w-lg">
				<form action={formAction}>
					<DialogHeader>
						<DialogTitle>{t("hoa.allocationKeys.weights.title", { name: customAllocationKeyLabel })}</DialogTitle>
						<DialogDescription>{t("hoa.allocationKeys.weights.description")}</DialogDescription>
					</DialogHeader>
					...
					<div className="grid gap-4 py-4">
						{units.length === 0 ? (
							<p className="text-sm text-muted-foreground">{t("hoa.allocationKeys.weights.noUnits")}</p>
						) : (
							units.map((unit) => (
