...
import { AttachmentError, extractAttachmentText } from "@/lib/ai/attachments";
...
describe("Anhang-Extraktion (src/lib/ai/attachments.ts)", () => {
	it("wandelt Excel-Arbeitsmappen in Semikolon-CSV-Text um (inkl. Datumswerten)", async () => {
		...
		const text = await extractAttachmentText("mieter.xlsx", Buffer.from(buffer).toString("base64"));

		expect(text).toContain('Tabellenblatt "Mieter"');
		expect(text).toContain("Name;Einheit;Miete;Beginn");
		expect(text).toContain("Max Mustermann;Whg 1;650.5;2026-01-01");
	});

	it("übernimmt Textdateien direkt", async () => {
		const text = await extractAttachmentText("notizen.csv", Buffer.from("a;b\n1;2", "utf8").toString("base64"));
		expect(text).toBe("a;b\n1;2");
	});

	it("lehnt das alte .xls-Format mit Hinweis ab", async () => {
		await expect(extractAttachmentText("alt.xls", Buffer.from("irgendwas").toString("base64"))).rejects.toThrow(AttachmentError);
		await expect(extractAttachmentText("alt.xls", Buffer.from("irgendwas").toString("base64"))).rejects.toThrow(/xlsx speichern/);
	});

	it("lehnt nicht unterstützte Dateitypen ab", async () => {
		await expect(extractAttachmentText("bild.png", Buffer.from("irgendwas").toString("base64"))).rejects.toThrow(/nicht unterstützt/);
	});

	it("kürzt übergroße Texte mit Hinweis", async () => {
		const text = await extractAttachmentText("gross.txt", Buffer.from("x".repeat(70_000)).toString("base64"));
