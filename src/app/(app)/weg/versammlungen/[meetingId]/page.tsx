...
import { ownerMeetingStatusLabels, ownerMeetingStatusStyles, ownerMeetingTypeLabels, resolutionVotingResultLabels, resolutionVotingResultStyles } from "@/lib/hoa-meetings";
...
export default async function OwnerMeetingDetailPage({ params }: { params: Promise<{ meetingId: string }> }) {
	...
	return (
		<div className="flex flex-1 flex-col">
			<SiteHeader
				title={meeting.title}
				description={`${meeting.hoaName} · ${ownerMeetingTypeLabels[meeting.type]}${meeting.meetingDate ? ` · ${formatDate(meeting.meetingDate)}` : ""}${meeting.location ? ` · ${meeting.location}` : ""}`}
				actions={
					<div className="flex items-center gap-2">
						...
						<span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${ownerMeetingStatusStyles[meeting.status]}`}>{ownerMeetingStatusLabels[meeting.status]}</span>
					</div>
				}
			/>
			<div className="flex-1 space-y-6 p-4 sm:p-6">
				...
				<div className="flex items-center justify-between">
					<h3 className="text-base font-semibold">Tagesordnung</h3>
					...
					<CardContent className="p-0">
						{agendaItems.length === 0 ? (
							<div className="flex flex-col items-center justify-center gap-2 py-8 text-center text-muted-foreground">
								...
								<p className="text-sm">Noch keine Tagesordnungspunkte erfasst.</p>
							</div>
						) : (
							<Table>
								<TableHeader>
										...
										<TableRow key={item.id}>
											...
											<TableCell>
												<div className="flex items-center justify-end gap-1">
													...
													<ConfirmDeleteButton action={deleteAgendaItemAction.bind(null, item.id, meeting.hoaId, meeting.id)} confirmMessage={`Tagesordnungspunkt "${item.title}" wirklich löschen?`} />
												</div>
											</TableCell>
										</TableRow>
									))}
				...
				{/* Einladung */}
				<div className="flex items-center justify-between">
					<h3 className="text-base font-semibold">Einladung</h3>
				</div>
				<Card>
				...
				{/* Beschlüsse */}
				<div className="flex items-center justify-between">
					<h3 className="text-base font-semibold">Beschlüsse</h3>
					...
					<CardContent className="p-0">
						{resolutions.length === 0 ? (
							<div className="flex flex-col items-center justify-center gap-2 py-8 text-center text-muted-foreground">
								...
								<p className="text-sm">Noch keine Beschlüsse erfasst.</p>
							</div>
						) : (
							<Table>
								<TableHeader>
								...
								<TableBody>
									{resolutions.map((resolution) => (
										<TableRow key={resolution.id} id={`resolution-${resolution.id}`}>
											...
											<TableCell>
												<span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${resolutionVotingResultStyles[resolution.votingResult]}`}>{resolutionVotingResultLabels[resolution.votingResult]}</span>
											</TableCell>
											<TableCell>
												<div className="flex items-center justify-end gap-1">
													<ResolutionFormDialog hoaId={meeting.hoaId} meetingId={meeting.id} agendaItems={agendaItems} resolution={resolution} />
													<ConfirmDeleteButton action={deleteResolutionAction.bind(null, resolution.id, meeting.hoaId, meeting.id)} confirmMessage={`Beschluss "${resolution.title}" wirklich löschen?`} />
												</div>
											</TableCell>
										</TableRow>
									))}
				...
				{/* Protokoll */}
				<div className="flex items-center justify-between">
					<h3 className="text-base font-semibold">Protokoll</h3>
				</div>
				<Card>
					<CardContent className="space-y-4 pt-6">
						...
						{meeting.minutesFinalizedAt ? (
							<p className="flex items-center gap-2 text-xs text-muted-foreground">
								...
								Protokoll finalisiert am {formatDate(meeting.minutesFinalizedAt)}.
							</p>
						) : null}
					</CardContent>
				</Card>
